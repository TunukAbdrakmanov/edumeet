...
def notes(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            notes.save()
        messages.success(request, f'Заметки добавлены от {request.user.username} успешно!')
    else:
        form = NotesForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
           ...
           homeworks = Homework(user=request.user, subject=request.POST['subject'],
                                ...
                                due_date=request.POST['due_date'], is_finished=finished)
        homeworks.save()
        messages.success(request, f'Домашнее задание добавлено от {request.user.username}')
    else:
        form = HomeworkForm()
    ...
    if request.method == 'POST':
        ...
        for i in range(10):
            result_dict = {
                ...
                'subtitle': data['items'][i]['volumeInfo'].get('subtitle'),
                ...
                'thumbnail': data['items'][i]['volumeInfo'].get('imageLinks').get('thumbnail'),
